...
public class MusicManager {
    ...
    public void load() {
        ...
        unlockedMusics.add(661);

        // Set the AUTO config.
        player.getPA().sendConfig(18, 1);
        player.getPA().sendFrame126("AUTO", 4439);

        // Update track list.
        updateTrackList();
    }
    int rgb555(int r, int g, int b) {
    ...
    public void updateRegionMusic(int region) {
        MusicLoader.Music music = MusicLoader.forRegion(region);
        if (!auto)
            return;
        if (music == null) {
            music = MusicLoader.forSong(2);
            player.frame74(music.getSong());
            player.getPA().sendFrame126("Region not added : "+player.getRegionId(), 4439);
        ...
        player.frame74(music.getSong());
        player.getPA().sendFrame126(music.getName(), 4439);
        if (!unlockedMusics.contains(music.getSong())) {
            unlockedMusics.add(music.getSong());
            player.sendMessage("You have unlocked a new music track: <col=ff0000>"+music.getName());
            player.frame122(music.getFrame(), ColorManager.convertRBG(0, 255, 0));
        }
    }
    ...
    public void updateTrackList() {
        for (MusicLoader.Music music : MusicLoader.getMusic()) {
            if (music == null)
                continue;


            if (!unlockedMusics.contains(music.getSong()))
                player.frame122(music.getFrame(), ColorManager.convertRBG(255, 0, 0));
            else
                player.frame122( music.getFrame(), ColorManager.convertRBG(0, 255, 0));
        }
    }
    ...
    public void handleMusicButtons(int button) {
        switch (button) {
            ...
            case 24126: // MAN feature.
                if (!man) {
                    ...
                    player.getPA().sendFrame126("MANUAL", 4439);
                }
                break;
        ...
        MusicLoader.Music music = MusicLoader.forFrame(button);
        if (music == null)
            return;
        if (!unlockedMusics.contains(music.getSong())) {

...
public class EventHandler {
    ...
    public <T> void stop(T attachment) {
        for (Event<?> event : active) {
            if (event.getPlr().equals(attachment)) {
                pendingRemoval.add(event);
            }
        }
        for (Event<?> event : pendingAddition) {
            if (event.getPlr().equals(attachment)) {
                pendingRemoval.add(event);
            }
        }
    ...
    public String toString() {
        ...
        pendingAddition.forEach(e -> sb.append("--> ").append(e.getPlr().toString()).append(" : ").append(e.getSignature()).append("\n"));
